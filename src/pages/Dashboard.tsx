...
import {
  User, TrendingUp, Crown, Clock, Calendar, Filter, ChevronRight,
  BarChart2, Shield, Activity, Lock, Brain, CheckCircle, AlertTriangle,
  Play, Ticket, Coins
} from 'lucide-react';

// --- CONFIGURATION SÉCURISÉE ---
// ✅ PLUS DE CLÉS API ICI - Elles sont maintenant sur le serveur
...
export default function Dashboard() {
  ...
  async function getProfile() {
    ...
    // Récupérer le nombre de paris du jour depuis localStorage
    const today = new Date().toISOString().split('T')[0];
    const savedBets = localStorage.getItem(`bets_${today}_${user.id}`);
    setBetsToday(savedBets ? parseInt(savedBets) : 0);
  }

  // --- 2. RÉCUPÉRATION MATCHS (API SÉCURISÉE) ---
  useEffect(() => {
    const fetchMatches = async () => {
      try {
        ...
        // ✅ APPEL SÉCURISÉ via notre API Route
        const res = await fetch(`/api/football?dateFrom=${dateFrom}&dateTo=${dateTo}`);
        const data = await res.json();

        if (data.matches) {
          setMatches(data.matches);
          setLiveMatches(data.matches.filter(m => 
  ...
  const limit = LIMITS[profile?.tier?.toUpperCase()] || LIMITS.NOVICE;

  // --- 3. LOGIQUE IA (APPEL SÉCURISÉ) ---
  const runAIAnalysis = async (item, type = 'SINGLE') => {
    const id = type === 'COMBO' ? `combo-${item[0].id}` : item.id;
    setAnalyzing(id);
    ...
    try {
      ...
      if (type === 'COMBO') {
        ...
        prompt = `Analyse ce COMBINÉ de matchs: ${matchNames}. Donne-moi une probabilité globale > 70% et une phrase de justification courte. Format JSON: {"analyse": "...", "probabilite": 85, "conseil": "Combiné Sécurisé"}`;
      } else {
        prompt = `Analyse rapide du match ${item.homeTeam.name} vs ${item.awayTeam.name}. Format JSON: {"analyse": "Phrase simple sur la forme", "probabilite": 75, "conseil": "Victoire ${item.homeTeam.shortName}"}`;
      }
        ...
        {LEAGUES.map(l => (
          <button
            ...
            style={{
              ...
              color: selectedLeague === l.code ? 'black' : '#94A3B8',
              fontSize: '0.8rem',
              whiteSpace: 'nowrap'
            }}
          >
      ...
      <div style={{ padding: '20px' }}>
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '15px' }}>
          ...
          <div style={{ fontSize: '0.8rem', color: '#94A3B8' }}>
            Paris restants: <span style={{ color: '#3B82F6', fontWeight: 'bold' }}>{limit - betsToday}</span>
          </div>
        </div>

        {loading ? (
          <div style={{ textAlign: 'center', color: '#94A3B8' }}>Chargement...</div>
            ...
            return (
              ...
              >
                ...
                {hasAnalysis ? (
                  ...
                  <div
                    onClick={() => runAIAnalysis(m, 'SINGLE')}
                    style={{
                      cursor: 'pointer',
                      fontSize: '0.8rem',
                      color: '#3B82F6',
                  ...
                  >
                    <Brain size={14} />
                    {analyzing === m.id ? 'Analyse en cours...' : 'Voir l\'analyse simple'}
                  </div>
                )}
                ...
                <button
                  ...
                  style={{
                    ...
                    color: betsToday >= limit ? '#94A3B8' : 'white',
                    cursor: betsToday >= limit ? 'not-allowed' : 'pointer'
                  }}
                >
  ...
  const renderVIP = () => {
    const combos = [];
    const vipMatches = matches.slice(0, 6);

    for (let i = 0; i < vipMatches.length; i += 2) {
      if (i + 1 < vipMatches.length) {
        combos.push([vipMatches[i], vipMatches[i + 1]]);
      }
    }
        ...
        {!isVip ? (
          <div style={{
            ...
            <p style={{ color: '#94a3b8', fontSize: '0.9rem', marginBottom: '20px' }}>
              Débloquez nos tickets combinés haute sécurité.
            </p>
            <button
          ...
          combos.map((combo, index) => {
            const comboId = `combo-${combo[0].id}`;
            ...
            return (
              <div
                ...
                style={{
                  ...
                  color: 'black',
                  overflow: 'hidden',
                  boxShadow: '0 5px 15px rgba(251, 192, 45, 0.2)'
                }}
              >
                {/* Header Ticket */}
                <div style={{
                ...
                <div style={{ padding: '15px' }}>
                  ...
                  {!analysis ? (
                    <button
                      onClick={() => runAIAnalysis(combo, 'COMBO')}
                      disabled={analyzing === comboId}
                      style={{
                        width: '100%',
                        marginTop: '10px',
                        padding: '10px',
                    ...
                    >
                      {analyzing === comboId ? 'Calcul de probabilité...' : '⚡ VÉRIFIER LA FIABILITÉ IA'}
                    </button>
                  ) : (
                    <div style={{ marginTop: '10px', background: 'rgba(255,255,255,0.8)', padding: '10px', borderRadius: '8px' }}>
                      <div style={{ display: 'flex', alignItems: 'center', gap: '5px', color: '#15803d', fontWeight: 'bold', marginBottom: '5px' }}>
                        <CheckCircle size={16} />
                        PROBABILITÉ: {analysis.probabilite}%
                      </div>
                      <div style={{ fontSize: '0.9rem', fontStyle: 'italic' }}>
                        {analysis.conseil} • {analysis.analyse}
                      </div>
                    </div>
                  )}
                </div>
  ...
  const renderLive = () => (
    <div style={{ padding: '20px', color: '#94A3B8', textAlign: 'center' }}>
      Live Score en maintenance pour optimisation.
    </div>
  );
    ...
    <div style={{ padding: '20px' }}>
      ...
      <div style={{
        ...
        <div>
          ...
          <div style={{
            color: isVip ? '#FFD700' : '#94A3B8',
            fontSize: '0.9rem',
            fontWeight: 'bold'
          }}>
            {isVip ? '👑 MEMBRE PREMIUM' : '🎯 PARIEUR DÉBUTANT'}
          </div>
        </div>
      </div>
  ...
  if (loading) {
    return (
      <div style={{ height: '100vh', display: 'flex', justifyContent: 'center', alignItems: 'center', background: '#0F172A', color: 'white' }}>
        <div className="animate-spin" style={{ width: '30px', height: '30px', border: '3px solid #00D9FF', borderRadius: '50%', borderTopColor: 'transparent' }}></div>
      </div>
    );
  }
    ...
    <div style={{ minHeight: '100vh', paddingBottom: '100px', background: '#0F172A', fontFamily: 'sans-serif' }}>
      ...
      <header style={{
        ...
        <div style={{ display: 'flex', gap: '10px', fontWeight: '900', alignItems: 'center', color: 'white', fontSize: '1.2rem' }}>
          <TrendingUp color="#00D9FF" />
          PASSION VIP
        </div>
        <div style={{ background: '#334155', padding: '5px 12px', borderRadius: '20px', fontSize: '0.8rem', fontWeight: 'bold', color: 'white' }}>
          {betsToday}/{limit} Paris
        </div>
      </header>
      ...
      {selectedMatchForBet && (
        <div
          ...
          onClick={() => setSelectedMatchForBet(null)}
        >
          <div
          ...
          >
            ...
            <div style={{ marginBottom: '20px', fontSize: '1.1rem', fontWeight: 'bold' }}>
              {selectedMatchForBet.homeTeam.shortName} vs {selectedMatchForBet.awayTeam.shortName}
            </div>

            {!betSuccess ? (
              <button
                onClick={() => {
                  const newBetsCount = betsToday + 1;
                  setBetsToday(newBetsCount);

                  // Sauvegarder dans localStorage
                  const today = new Date().toISOString().split('T')[0];
                  const userId = profile?.id || 'guest';
                  localStorage.setItem(`bets_${today}_${userId}`, newBetsCount.toString());
